...
- Nine Node tests covering dice thresholds, attribute-vs-skill/power modifiers, allocation validation, duplicate display names, condition formulas, Actor creation/editing, legacy backups/preservation, ownership, creation permissions, and stale editor/session checks, and sheet-header Actor targeting/ownership/editor reuse.
...
Version 0.1.1: TypeScript, production build and nine Node tests passed. The new header control has not been tested on a live Foundry server. React DOM interaction results above refer to the 0.1.0 baseline.
