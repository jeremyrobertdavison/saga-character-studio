...
2. Create tag `v0.1.0` targeting the committed default branch.
3. Title it `SAGA Character Studio v0.1.0 — Initial Playtest`.
4. Describe the features and state that live Foundry testing remains outstanding.
5. Attach both `saga-character-studio-v0.1.0.zip` and `module.json` as release assets. The manifest must match the copy inside the ZIP.
...
- https://github.com/jeremyrobertdavison/saga-character-studio/releases/download/v0.1.0/saga-character-studio-v0.1.0.zip
...
2. Update `version` in `module.json`, e.g. `0.1.1`.
